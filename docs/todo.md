...
> 
> **Spring Modulith:** DTO знаходяться в `adoption/dto/` (public API), а не в `adoption/application/dto/`. Сервісні інтерфейси (`AdoptionPostService`, etc.) також в корені `adoption/` — це публічний API модуля.
